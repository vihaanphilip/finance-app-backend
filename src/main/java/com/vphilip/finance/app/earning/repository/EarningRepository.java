...
public interface EarningRepository extends JpaRepository<Earning, Long> {
    ...
    @Query(value = """
            ...
            LEFT JOIN earning_category ec ON e.earning_category_id = ec.id
            LEFT JOIN earning_type et ON et.id = ec.earning_type_id
            WHERE e.budget_id = :budgetId AND a.user_id = :userId
            ORDER BY e.id DESC
            """, nativeQuery = true)
    List<EarningDTO> findAllByBudgetId(@Param("budgetId") Long budgetId, @Param("userId") Integer userId);

    @Modifying
    @Query(value = "UPDATE earning SET budget_id = NULL WHERE budget_id = :budgetId", nativeQuery = true)
